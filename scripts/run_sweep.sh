...
print_usage() {
    echo "Usage: $0 [--selected_gpus GPU_IDS] [--sweep_config SWEEP_CONFIG_PATH] [--base_config BASE_CONFIG_PATH] [--count COUNT] [--resume SWEEP_ID]"
    echo ""
    echo "Example:"
    ...
    echo "  --resume          Resume an existing sweep using the provided Sweep ID (skips sweep creation)"
...
if [ -n "${RESUME_SWEEP_ID}" ]; then
    SWEEP_ID="${RESUME_SWEEP_ID}"
    echo -e "${BLUE}Resuming existing W&B Sweep with ID: ${SWEEP_ID}${NC}"
else
    ...
    if [ -z "${SWEEP_ID}" ]; then
        ...
        echo -e "${YELLOW}Sweep creation output:${NC}"
        echo "${SWEEP_OUTPUT}"
        echo -e "${BLUE}========================================${NC}"
        exit 1
    fi
