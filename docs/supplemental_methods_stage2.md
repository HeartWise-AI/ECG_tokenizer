...
The Q-Former bridge (198.7M non-text bridge parameters) is a dual-stream transformer comprising 10 base layers and 10 cross-modal Stage-1 layers, with hidden dimension 768 and 8 attention heads (head dimension 96). Dropout of 0.1 is applied throughout.

**Input embedding.** Codes from all 8 available QINCo/VQ codebooks are embedded through 8 codebook-specific embedding tables (512 codes x 768 dim each). A learned gating MLP (4.7M parameters) mixes codebook embeddings at each timestep via sigmoid-gated interpolation, producing a single 768-dimensional representation per temporal position. The 82-length latent sequence from the ECG tokenizer encoder is thus embedded into an 82 x 768 matrix.

**Query tokens.** 16 learnable query tokens (initialized as $\mathcal{N}(0, 1/\sqrt{768})$) serve as the bridge's compressed ECG representation. At each transformer layer, query tokens and instruction text token embeddings are concatenated and processed through joint self-attention. Cross-attention to the frozen ECG encoder output is applied every 2 layers (layers 0, 2, 4, 6, 8), restricted to query tokens only — text tokens attend to queries through self-attention but not directly to ECG embeddings.

**Output projections.** Query outputs are pooled via a gated pooling mechanism (RMSNorm followed by sigmoid gate) and projected to the 768-dimensional Stage-2 text space through a linear layer (0.6M parameters). Separate projection heads produce ECG-side and text-side embeddings for the contrastive and matching objectives.
...
**Focal weighting.** Focal InfoNCE weighting was included in the sweep but disabled in the selected `j4bb0w33` checkpoint (`focal_infonce = false`). Rare label support is instead handled through the tail-class sampling and reporting pathway described below.
...
**Dynamic in-batch mining (k=3).** For each ECG in the mini-batch, we compute cosine similarities to all text embeddings using current ETC representations. After masking self-pairs, same-label pairs, and same-ECG pairs, we select the three most similar remaining texts as hard in-batch negatives. This forces the bridge to learn fine-grained distinctions between clinically similar conditions.
...
|---|---|---|
| Bridge transformer layers | 10 | {6, 8, 10} |
| Hidden dimension | 768 | — |
| Attention heads | 8 | {8, 12} |
| Query tokens | 16 | {16, 32, 48} |
| Cross-attention frequency | Every 2 layers | {1, 2} |
| Codebooks kept | 8 (of 8) | {1, 2, 4, 8} |
| Codebook size | 512 codes | — |
| Codebook offset | 0 (use all codebooks from first) | — |
| Learning rate | 1.4 x 10^-4 | log-uniform [5 x 10^-5, 4 x 10^-4] |
| Weight decay | 0.01 | {0, 0.005, 0.01, 0.02} |
| Optimizer | AdamW (beta1=0.9, beta2=0.999) | — |
| Batch size | 96 | {64, 96, 120} |
| Epochs | 10 | — |
| Precision | bfloat16 | — |
| Gradient accumulation | 1 | — |
| Max text length | 128 tokens | — |
| Hard negative k | 3 | {1, 2, 3} |
| ETM warmup steps | 2,000 | — |
| ETG delay steps | 2,000 | — |
| ETG warmup to full weight | 4,000 steps | — |
| Focal InfoNCE | Disabled | {false, true} |
| Focal gamma (positive) | 1.0 (inactive in final run) | {1.0, 1.5, 2.0} |
| Focal gamma (negative) | 0.25 (inactive in final run) | {0.0, 0.25, 0.5} |
...
Stage 2 training used 761,386 unique MHI ECGs, and the ECG-text pairs were built by expanding each ECG across the diagnostic text targets that applied to it from a bank of 222 text entries (74 atomic label descriptions + 148 binary QA pairs covering 74 pathological conditions across 6 diagnostic categories). This many-to-many expansion produced 9,565,410 ECG-text training pairs, with a mean of 12.6 text IDs per ECG. This construction is required because ETC learns ECG-text alignment, ETM needs positive pairs plus hard negatives for confusion-resistant matching, and ETG needs target text for generation; a single full-report target would not provide enough supervision for these objectives. In practice, an ECG with two positive findings would pair with the atomic descriptions and yes/no QA prompts corresponding to those findings. Text entries follow two granularities:
...
Validation used 94,948 pairs from 7,565 held-out ECGs. The text encoder (MedGemma 4B-IT) was fully frozen throughout ($\text{freeze\_ratio} = 1.0$, $\text{lr\_multiplier} = 0.0$); only the Q-Former bridge parameters were updated.

### Hyperparameter Selection

Hyperparameters were selected via Bayesian sweep (W&B Sweeps) with Hyperband early stopping ($\text{min\_iter} = 3$, $\eta = 2$). The sweep explored 12 hyperparameters across the ranges specified in Table S4. The best configuration (run `j4bb0w33`) was selected based on validation contrastive loss; its best epoch-1 checkpoint was used to initialize the instruction-aware Q-Former in Stage 3.

## Stage 3: Instruction Tuning

### Architecture Modifications

The Stage 3 instruction-aware bridge retained the Stage 2 all-codebook input and 10-layer depth, while increasing the instruction-aware bridge capacity to 12 heads and 32 query tokens. Two additional architectural changes were made:
...
| Instruction-aware bridge | 10 layers, 12 heads, 32 queries, 768 dim |
...
| Bridge parameters | 200.1M |
| Total trainable | 223.9M (of 4.3B total) |
...
1. **Phase 1 — Bridge alignment** (1 epoch): The LLM backbone was frozen; only the Q-Former bridge and LoRA adapters were updated. This allows the instruction-aware bridge, initialized from the all-codebook 10-layer Stage 2 checkpoint, to adapt to instruction-conditioned ECG feature extraction before the LLM fine-tuning begins.
