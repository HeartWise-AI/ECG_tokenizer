...
@ModelRegistry.register("OPT_WithEmbedding")
class OPTWithEmbedding(nn.Module):
    def __init__(
        self, 
        opt_model_name: str = 'facebook/opt-350m', 
        opt_embedding_size: int = 512, 
        ecg_embedding_size: tuple[int, int, int] = (8, 128, 82),
        reducer_name: str = None,
        reducer_dropout: float = 0.2
    ):
        super(OPTWithEmbedding, self).__init__()
        self.opt: OPTForCausalLM = OPTForCausalLM.from_pretrained(opt_model_name)
        self.embedding_reducer: Union[
            ...
            output_size=opt_embedding_size, 
            dropout=reducer_dropout
        )
        
        # Check if embedding size matches OPT's hidden size.
        if opt_embedding_size != self.opt.config.hidden_size:
            raise ValueError(f"Embedding size {opt_embedding_size} does not match OPT hidden size {self.opt.config.hidden_size}")
        ...
        self.opt.resize_token_embeddings(len(self.opt.get_input_embeddings().weight) + 1)
        self.ecg_token_id = len(self.opt.get_input_embeddings().weight) - 1  # New token ID
        # Make sure EOS token is defined
        self.eos_token_id = self.opt.config.eos_token_id
    ...
    def forward(
        ...
        """
        Forward pass for the OPTWithEmbedding model.
        ...
        Returns:
            Dictionary containing loss, logits, and other outputs from the OPT model
        ...
        input_embedding = self.opt.get_input_embeddings()(input_ids)  # (batch, seq_length + 1, hidden_size)
        input_embedding[:, 0, :] = reduced  # Replace ECG token embedding
        
        # Forward pass through OPT
        outputs = self.opt(
    ...
    def generate_report(
        ...
        **generate_kwargs
        ...
        This method uses OPT's generate() function with inputs_embeds.
        ...
        input_embedding = self.opt.get_input_embeddings()(ecg_token)  # (batch, 1, hidden_size)
        input_embedding[:, 0, :] = reduced  # Replace ECG token embedding
