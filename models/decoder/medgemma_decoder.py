...
class MedGemmaDecoder(nn.Module):
    ...
    def generate_blip2_style(
        ...
        *,
        ...
        """BLIP-2 compatible generate wrapper.

        Mirrors the LAVIS Blip2T5.generate signature by:
        - Building a prompt tensor from the provided `prompt` string(s)
        - Concatenating learned ECG prefix embeddings with text embeddings
        - Routing decoding args to HuggingFace `.generate()`

        Returns a list of decoded strings of length batch_size * num_captions.
        """
        embed_layer = self.llm_model.get_input_embeddings()
        device = embed_layer.weight.device

        # Resolve batch size from ECG inputs
        if getattr(self.bridge, "uses_codes", False):
            if quantized_codes is None:
                raise ValueError("quantized_codes must be provided when using the ECG code bridge")
            batch_size = int(quantized_codes.size(0))
        else:
            if quantized_features is None:
                raise ValueError("quantized_features must be provided when using the ECG projection bridge")
            batch_size = int(quantized_features.size(0))

        # Build prompt ids/masks in a Gemma/MedGemma-friendly way
        if prompt is None:
            prompt_ids, prompt_mask = self._build_default_prompt(batch_size, device)
        else:
            prompts: list[str]
            if isinstance(prompt, str):
                ...
                if len(prompt) != batch_size:
                    raise ValueError("The number of prompts must equal the batch size.")
                prompts = list(prompt)

            # Wrap as chat turns to honor tokenizer templates
            rendered: list[str] = []
            for p in prompts:
                messages = [
                    {"role": "system", "content": "You are an expert cardiologist. Read the ECG patches and answer concisely."},
                    {"role": "user", "content": f"<start_of_image> {p}"},
                ]
                rendered.append(
                    str(
                        self.tokenizer.apply_chat_template(  # type: ignore[attr-defined]
        ...
        # Compute ECG prefix embeddings (detached for stability in pure generation)
        ecg_embeddings, _ = self._compute_ecg_embeddings(
            quantized_features,
            quantized_codes,
        ...
        if merged is not None:
            inputs_embeds, attention_mask, prompt_ids, _ = merged
        else:
        ...
        # Map BLIP-2 style args to HF generate
        gen_args = dict(self.default_generation_params)
        gen_args.update(
            {
                "do_sample": bool(use_nucleus_sampling),
                "top_p": float(top_p),
                "temperature": float(temperature),
                "num_beams": int(max(1, num_beams)),
                "max_new_tokens": int(max_length),
                "min_new_tokens": int(max(0, min_length)),
                "repetition_penalty": float(repetition_penalty),
                "length_penalty": float(length_penalty),
                "num_return_sequences": int(max(1, num_captions)),
            }
        )
        if self.bad_ecg_token_ids:
            gen_args.setdefault("bad_words_ids", self.bad_ecg_token_ids)
        else:
            gen_args.pop("bad_words_ids", None)

        gen_args = self._sanitize_generate_args(gen_args)
        ...
        # Decode text like BLIP-2 does
        texts = self.tokenizer.batch_decode(outputs, skip_special_tokens=True)
        return texts

    def predict_answers_blip2_style(
        self,
        quantized_features: Optional[torch.Tensor] = None,
        quantized_codes: Optional[torch.Tensor] = None,
        *,
        ...
        """BLIP-2 style answer prediction for QA prompts.

        - If `prompt` contains a `{}` placeholder, it formats each question into the prompt string.
        - Otherwise, `text_input` is used as-is.
        """
        if isinstance(text_input, str):
            inputs = [text_input]
        else:
            inputs = list(text_input)

        if prompt:
            prompts = [prompt.format(q) for q in inputs]
        else:
            prompts = inputs

        return self.generate_blip2_style(
