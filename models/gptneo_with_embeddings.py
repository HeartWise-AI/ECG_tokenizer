...
@ModelRegistry.register("GPTNeo_WithEmbedding")
class GPTNeoWithEmbedding(nn.Module):
    def __init__(
        self, 
        gptneo_model_name: str = 'EleutherAI/gpt-neo-125M', 
        gptneo_embedding_size: int = 768, 
        ecg_embedding_size: tuple[int, int, int] = (8, 128, 82),
        reducer_name: str = None,
        reducer_dropout: float = 0.2
    ):
        super(GPTNeoWithEmbedding, self).__init__()
        self.gptneo: GPTNeoForCausalLM = GPTNeoForCausalLM.from_pretrained(gptneo_model_name)
        self.embedding_reducer: Union[
            ...
            output_size=gptneo_embedding_size, 
            dropout=reducer_dropout
        )
        
        # Check if embedding size matches GPT-Neo's hidden size.
        if gptneo_embedding_size != self.gptneo.config.hidden_size:
            raise ValueError(f"Embedding size {gptneo_embedding_size} does not match GPT-Neo hidden size {self.gptneo.config.hidden_size}")
        ...
        self.gptneo.resize_token_embeddings(len(self.gptneo.get_input_embeddings().weight) + 1)
        self.ecg_token_id = len(self.gptneo.get_input_embeddings().weight) - 1  # New token ID
        # Make sure EOS token is defined
        self.eos_token_id = self.gptneo.config.eos_token_id
    ...
    def forward(
        ...
        """
        Forward pass for the GPTNeoWithEmbedding model.
        ...
        Returns:
            Dictionary containing loss, logits, and other outputs from the GPT-Neo model
        ...
        input_embedding = self.gptneo.get_input_embeddings()(input_ids)  # (batch, seq_length + 1, hidden_size)
    ...
    def generate_report(
        ...
        **generate_kwargs
        ...
        This method uses GPT-Neo's generate() function with inputs_embeds.
        ...
        input_embedding = self.gptneo.get_input_embeddings()(ecg_token)  # (batch, 1, hidden_size)
        input_embedding[:, 0, :] = reduced  # Replace ECG token embedding
