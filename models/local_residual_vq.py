...
class ResidualVQ(Module):
    ...
    def forward(
        ...
        # NEW: Collect quantized outputs directly during forward pass
        all_quantized_codes = [] if return_all_codes else None
        ...
        for quantizer_index, (vq, maybe_mlp) in enumerate(zip(self.layers, maybe_code_transforms)):
            ...
            if should_quantize_dropout and quantizer_index > rand_quantize_dropout_index:
                ...
                # NEW: Add null quantized codes for dropped layers
                if return_all_codes:
                    null_quantized = torch.zeros_like(residual)
                    all_quantized_codes.append(null_quantized)
            ...
            # NEW: Collect quantized outputs directly during forward pass
            if return_all_codes:
                all_quantized_codes.append(quantized.clone())
        ...
        if return_all_codes:
            # NEW: Return the collected quantized codes instead of calling expensive get_codes_from_indices
            all_codes = torch.stack(all_quantized_codes)  # Shape: (num_quantizers, batch, seq_len, dim)
