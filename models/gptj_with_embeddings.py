...
@ModelRegistry.register("GPTJ_WithEmbedding")
class GPTJWithEmbedding(nn.Module):
    def __init__(
        self, 
        gptj_model_name: str = 'EleutherAI/gpt-j-6B', 
        gptj_embedding_size: int = 4096, 
        ecg_embedding_size: tuple[int, int, int] = (8, 128, 82),
        reducer_name: str = None,
        reducer_dropout: float = 0.2
    ):
        super(GPTJWithEmbedding, self).__init__()
        self.gptj: GPTJForCausalLM = GPTJForCausalLM.from_pretrained(gptj_model_name)
        self.embedding_reducer: Union[
            ...
            output_size=gptj_embedding_size, 
            dropout=reducer_dropout
        )
        
        # Check if embedding size matches GPT-J's hidden size.
        if gptj_embedding_size != self.gptj.config.n_embd:
            raise ValueError(f"Embedding size {gptj_embedding_size} does not match GPT-J hidden size {self.gptj.config.n_embd}")
        ...
        self.gptj.resize_token_embeddings(len(self.gptj.get_input_embeddings().weight) + 1)
        self.ecg_token_id = len(self.gptj.get_input_embeddings().weight) - 1  # New token ID
        # Make sure EOS token is defined
        self.eos_token_id = self.gptj.config.eos_token_id
    ...
    def forward(
        ...
        """
        Forward pass for the GPTJWithEmbedding model.
        ...
        Returns:
            Dictionary containing loss, logits, and other outputs from the GPT-J model
        ...
        input_embedding = self.gptj.get_input_embeddings()(input_ids)  # (batch, seq_length + 1, hidden_size)
    ...
    def generate_report(
        ...
        **generate_kwargs
        ...
        This method uses GPT-J's generate() function with inputs_embeds.
        ...
        input_embedding = self.gptj.get_input_embeddings()(ecg_token)  # (batch, 1, hidden_size)
        input_embedding[:, 0, :] = reduced  # Replace ECG token embedding
