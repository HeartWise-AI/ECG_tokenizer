...
@ModelRegistry.register("BLOOM_WithEmbedding")
class BloomWithEmbedding(nn.Module):
    def __init__(
        self, 
        bloom_model_name: str = 'bigscience/bloom-560m', 
        bloom_embedding_size: int = 1024, 
        ecg_embedding_size: tuple[int, int, int] = (8, 128, 82),
        reducer_name: str = None,
        reducer_dropout: float = 0.2
    ):
        super(BloomWithEmbedding, self).__init__()
        self.bloom: BloomForCausalLM = BloomForCausalLM.from_pretrained(bloom_model_name)
        self.embedding_reducer: Union[
            ...
            output_size=bloom_embedding_size, 
            dropout=reducer_dropout
        )
        
        # Check if embedding size matches BLOOM's hidden size.
        if bloom_embedding_size != self.bloom.config.hidden_size:
            raise ValueError(f"Embedding size {bloom_embedding_size} does not match BLOOM hidden size {self.bloom.config.hidden_size}")
        ...
        self.bloom.resize_token_embeddings(len(self.bloom.get_input_embeddings().weight) + 1)
        self.ecg_token_id = len(self.bloom.get_input_embeddings().weight) - 1  # New token ID
        # Make sure EOS token is defined
        self.eos_token_id = self.bloom.config.eos_token_id
    ...
    def forward(
        ...
        """
        Forward pass for the BloomWithEmbedding model.
        ...
        Returns:
            Dictionary containing loss, logits, and other outputs from the BLOOM model
        ...
        input_embedding = self.bloom.get_input_embeddings()(input_ids)  # (batch, seq_length + 1, hidden_size)
        input_embedding[:, 0, :] = reduced  # Replace ECG token embedding
        
        # Forward pass through BLOOM
        outputs = self.bloom(
    ...
    def generate_report(
        ...
        **generate_kwargs
        ...
        This method uses BLOOM's generate() function with inputs_embeds.
        ...
        input_embedding = self.bloom.get_input_embeddings()(ecg_token)  # (batch, 1, hidden_size)
        input_embedding[:, 0, :] = reduced  # Replace ECG token embedding
