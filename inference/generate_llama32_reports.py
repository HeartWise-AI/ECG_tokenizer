#!/usr/bin/env python3
"""
Run standalone Llama-3.2-1B ECG report generation on a validation parquet.

Parallel to inference/generate_gpt2_reports.py: loads a finetuned
ECG_Tokenizer_Wrapper checkpoint that wraps a Llama32ReportDecoder, reads
ECG waveforms from the validation parquet, generates one clinical report per
ECG via the manual autoregressive loop, and writes CSV and JSON outputs.

Example:
    CUDA_VISIBLE_DEVICES=0 python inference/generate_llama32_reports.py \
        --checkpoint /volume/ECG_tokenizer/checkpoints/ECG_Tokenizer_LLM_Finetuning/.../checkpoint_epoch_1.pt \
        --output_dir /volume/ECG_tokenizer/checkpoints/ECG_Tokenizer_LLM_Finetuning/.../llama32_report_inference
"""
...
# Import package side effects so ModelRegistry contains Llama32ReportDecoder,
# SequenceAdapter and ECG_Tokenizer_Wrapper.
...
def load_model(checkpoint_path: str, device: torch.device, tokenizer_path: str | None = None):
    ...
    tokenizer_name = tokenizer_path or str(_cfg_get(config, "tokenizer_name", "meta-llama/Llama-3.2-1B"))
    ...
    adapter_name = str(
        _cfg_get(
            config,
            "adapter_name",
            _cfg_get(config, "bridge_name", "Llama32_SequenceAdapter"),
        )
    )
    ...
    model = ECG_Tokenizer_Wrapper(
        ...
        decoder_name=str(_cfg_get(config, "decoder_name", "Llama32_Decoder")),
        ...
        llm_input_embedding_size=int(_cfg_get(config, "llm_input_embedding_size", 2048)),
    ...
    model.eval()
    model.to(device)
    print(f"Loaded model on {device} with adapter={adapter_name}, decoder=Llama32_Decoder")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate Llama-3.2-1B ECG reports for a validation parquet.")
    parser.add_argument("--checkpoint", required=True, help="Path to finetuned Llama-3.2-1B checkpoint.")
    parser.add_argument(
        "--validation_parquet",
        default=None,
    ...
    parser.add_argument(
        "--output_dir",
        default=None,
        help="Output directory. Defaults to <checkpoint_dir>/llama32_report_inference.",
    )
    parser.add_argument("--output_prefix", default="llama32_report_generations")
    ...
    parser.add_argument("--tokenizer_path", default=None, help="Optional tokenizer/model path override.")
...
def main() -> pd.DataFrame:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else checkpoint_path.parent / "llama32_report_inference"
    ...
    pbar = tqdm(total=len(df), desc="Llama-3.2-1B report inference")
