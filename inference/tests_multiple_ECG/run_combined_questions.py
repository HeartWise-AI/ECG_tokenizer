#!/usr/bin/env python3
"""
Run combined questions (multiple questions in one prompt) on ECGs.

Usage:
    CUDA_VISIBLE_DEVICES=0 python inference/run_combined_questions.py \
...
COMBINED_QUESTION = """Please answer both questions:
1. What is the rhythm? Is there anything else I should know or do?
2. Are there any ST segment changes? If so, describe them."""
...
def main():
    parser = argparse.ArgumentParser(description="Run combined questions on ECGs")
    ...
    parser.add_argument("--num_ecgs", type=int, default=50)
    ...
    for idx, row in tqdm(unique_ecgs.iterrows(), total=len(unique_ecgs), desc="Processing ECGs"):
        ...
        try:
            ...
            answer = generate_answer(
                model, tokenizer, ecg_tensor, COMBINED_QUESTION, device,
                temperature=args.temperature,
                max_new_tokens=256,  # More tokens for multi-part answer
            )
    ...
    output_prefix = f"combined_2questions_{args.num_ecgs}ecgs_temp{args.temperature}"
